...
var WithdrawalGateway = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers"


// 企业付款到零钱 请求参数
type WithdrawalRequest struct {
	Mch_appid        string `json:"mch_appid"  xml:"mch_appid"`              //  必传       申请商户号的 appid  或  商户号 绑定的 appid
	...
	Openid           string `xml:"openid" json:"openid"`                     //  必传       商户 appid 下 ，某用户的 openid
...
func (pcf *Pay) Withdrawal(req  *WithdrawalRequest)(rsp *WithdrawalResponse, err error){
	...
	rawRet ,err := util.PostXMLWithTLS(WithdrawalGateway,request,req.RootCa,req.Mchid)
	if err != nil {
		return
	}
	...
	if rsp.ReturnCode == "SUCCESS" {
		...
		err = fmt.Errorf("withdrawal error, errcode=%s,errmsg=%s", rsp.ErrCode, rsp.ErrCodeDes)
		return
	}
